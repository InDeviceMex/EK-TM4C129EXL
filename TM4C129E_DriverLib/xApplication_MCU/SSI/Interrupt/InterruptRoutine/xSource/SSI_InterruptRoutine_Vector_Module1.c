...
void SSI1__vIRQVectorHandler(void)
{
    ...
    if((uint32_t) SSI_enINT_SOURCE_RECEIVE_OVERRUN & u32Reg)
    {
        ...
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_RECEIVE_OVERRUN);
        pfvCallback();
    }
    if((uint32_t) SSI_enINT_SOURCE_RECEIVE_TIMEOUT & u32Reg)
    {
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_RECEIVE_TIMEOUT);
        pfvCallback();
        SSI1_ICR_R = (uint32_t) SSI_enINT_SOURCE_RECEIVE_TIMEOUT;
    }
    if((uint32_t) SSI_enINT_SOURCE_RECEIVE & u32Reg)
    {
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_RECEIVE);
        pfvCallback();
    }
    if((uint32_t) SSI_enINT_SOURCE_TRANSMIT & u32Reg)
    {
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_TRANSMIT);
        pfvCallback();
    }
    if((uint32_t) SSI_enINT_SOURCE_RECEIVE_DMA & u32Reg)
    {
        SSI1_ICR_R = (uint32_t) SSI_enINT_SOURCE_RECEIVE_DMA;
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_RECEIVE_DMA);
        pfvCallback();
    }
    if((uint32_t) SSI_enINT_SOURCE_TRANSMIT_DMA & u32Reg)
    {
        ...
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_TRANSMIT_DMA);
        pfvCallback();
    }
    if((uint32_t) SSI_enINT_SOURCE_END_OF_TRANSMIT & u32Reg)
    {
        SSI1_ICR_R = (uint32_t) SSI_enINT_SOURCE_END_OF_TRANSMIT;
        pfvCallback = SSI__pvfGetIRQSourceHandler(SSI_enMODULE_1, SSI_enINTERRUPT_END_OF_TRANSMIT);
        pfvCallback();
    }
}
