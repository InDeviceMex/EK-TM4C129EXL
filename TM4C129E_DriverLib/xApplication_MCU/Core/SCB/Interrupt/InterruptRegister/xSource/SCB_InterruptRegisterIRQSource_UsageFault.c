...
SCB_nERROR SCB_UsageFault__enRegisterIRQSourceHandler(SCB_nMODULE enModuleArg, SCB_nUSAGE_BIT enInterrupArg,
                                                       SCB_pvfIRQSourceHandler_t pfIrqSourceHandlerArg)
{
    ...
    {
        enErrorReg = (SCB_nERROR) MCU__enCheckParams((uint32_t) enInterrupArg, (uint32_t) SCB_enUSAGE_BIT_MAX);
    }
    if(SCB_enERROR_OK == enErrorReg)
    {
        pvfIrqHandler = SCB_UsageFault__pvfGetIRQSourceHandlerPointer(enModuleArg, enInterrupArg);
