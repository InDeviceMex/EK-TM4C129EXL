...
void GPIOJ__vIRQVectorHandler(void)
{
    ...
    if(SYSCTL_RCGCGPIO_R_GPIOJ_DIS == (SYSCTL_RCGCGPIO_R_GPIOJ_MASK & u32Enable))
    {
        pfvCallback = GPIO_SW__pvfGetIRQSourceHandler(GPIO_enPORT_J);
        pfvCallback();
    }
    else
    {
        ...
        else
        {
            if((uint32_t) GPIO_MIS_R_DMAMIS_OCCUR & u32Reg)
            {
                ...
                pfvCallback = GPIO_DMA__pvfGetIRQSourceHandler(GPIO_enPORT_J);
                pfvCallback();
            }
