    ...
    else
    {
        ...
        else
        {
            if((uint32_t) GPIO_MIS_R_DMAMIS_OCCUR & u32Reg)
            {
                ...
                pvfCallback = GPIO_DMA__pvfGetIRQSourceHandler(GPIO_enPORT_F);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_0 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_0;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER0);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_1 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_1;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER1);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_2 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_2;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER2);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_3 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_3;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER3);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_4 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_4;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER4);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_5 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_5;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER5);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_6 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_6;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER6);
                pvfCallback();
            }
            if((uint32_t) GPIO_enPIN_7 & u32Reg)
            {
                GPIOF_ICR_R = (uint32_t) GPIO_enPIN_7;
                pvfCallback = GPIO__pvfGetIRQSourceHandler(GPIO_enPORT_F, GPIO_enPIN_NUMBER7);
                pvfCallback();
            }
        }
    }
