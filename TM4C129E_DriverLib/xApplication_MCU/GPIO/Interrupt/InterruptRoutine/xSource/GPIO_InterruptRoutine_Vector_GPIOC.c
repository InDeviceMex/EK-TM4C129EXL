...
void GPIOC__vIRQVectorHandler(void)
{
    ...
    if(SYSCTL_RCGCGPIO_R_GPIOC_DIS == (SYSCTL_RCGCGPIO_R_GPIOC_MASK & u32Enable))
    {
        pfvCallback = GPIO_SW__pvfGetIRQSourceHandler(GPIO_enPORT_C);
        pfvCallback();
    }
    else
    {
        ...
        else
        {
            if((uint32_t) GPIO_MIS_R_DMAMIS_OCCUR & u32Reg)
            {
                ...
                pfvCallback = GPIO_DMA__pvfGetIRQSourceHandler(GPIO_enPORT_C);
                pfvCallback();
            }
