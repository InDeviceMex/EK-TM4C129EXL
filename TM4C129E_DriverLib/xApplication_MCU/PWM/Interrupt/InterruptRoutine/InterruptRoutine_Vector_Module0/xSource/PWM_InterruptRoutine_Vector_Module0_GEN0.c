...
void PWM0_GEN0__vIRQVectorHandler(void)
{
    ...
    if(SYSCTL_PRPWM_R_PWM0_NOREADY == (SYSCTL_PRPWM_R_PWM0_MASK & u32Ready))
    {
        pvfCallback = PWM_Generator__pvfGetIRQSourceHandler(PWM_enMODULE_0,
                                                     PWM_enGEN_0, PWM_enGEN_INTERRUPT_SW);
        pvfCallback();
    }
    else
    {
        u32Reg = PWM0_ISC_R;
        u32RegSource = PWM0_GEN0_ISC_R;
        if(0UL == ((uint32_t) PWM_enGENMASK_0 &u32Reg))
        {
            pvfCallback = PWM_Generator__pvfGetIRQSourceHandler(PWM_enMODULE_0,
                                                         PWM_enGEN_0, PWM_enGEN_INTERRUPT_SW);
            pvfCallback();
        ...
        else
        {
            ...
            {
                PWM0_GEN0_ISC_R = (uint32_t) PWM_enGEN_INT_SOURCE_ZERO;
                pvfCallback = PWM_Generator__pvfGetIRQSourceHandler(PWM_enMODULE_0,
                                                                 PWM_enGEN_0,
            ...
            {
                PWM0_GEN0_ISC_R = (uint32_t) PWM_enGEN_INT_SOURCE_LOAD;
                pvfCallback = PWM_Generator__pvfGetIRQSourceHandler(PWM_enMODULE_0,
                                                                 PWM_enGEN_0,
