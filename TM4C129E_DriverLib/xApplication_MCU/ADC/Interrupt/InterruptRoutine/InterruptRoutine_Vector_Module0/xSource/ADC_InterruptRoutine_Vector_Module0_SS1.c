...
void ADC0_SS1__vIRQVectorHandler(void)
{
    ...
    {
        u32Reg = ADC0_ISC_R;
        if(0UL == ((ADC_ISC_R_DMAIN1_MASK | ADC_ISC_R_IN1_MASK | ADC_ISC_R_DCINSS1_MASK ) &u32Reg))
        {
            pvfCallback = ADC_SW__pvfGetIRQSourceHandler(ADC_enMODULE_0,
                                                         ADC_enSEQ_1);
            pvfCallback(ADC0_BASE, (void*) ADC_enSEQ_1);
        ...
        {
            if(u32Reg & ADC_ISC_R_DMAIN1_MASK)
            {
                ADC0_ISC_R =  ADC_ISC_R_DMAIN1_CLEAR;
                pvfCallback = ADC_Sequencer__pvfGetIRQSourceHandler(ADC_enMODULE_0,
                                                                 ADC_enSEQ_1,
                                                                 ADC_enINT_TYPE_DMA);
                pvfCallback(ADC0_BASE, (void*) ADC_enSEQ_1);
            }
            if(u32Reg & ADC_ISC_R_IN1_MASK)
            {
            ...
            if(u32Reg & ADC_ISC_R_DCINSS1_MASK)
            {
                ...
                u32RegCompMux = ADC0_SS1_DC_R;
                u32RegCompSelect = ADC0_SS1_OP_R;
                u32ShiftReg = 0x1UL;
                    ...
                    if(u32RegCompSelect & u32ShiftReg)
                    {
                        ...
                        if(u32RegCompInterrupt & u32RegCompMuxBit)
                        {
                            ...
                            if(u32RegCompEnable & ADC_DC_CTL_R_CIE_MASK)
                            {
                                ...
                                pvfCallback = ADC_Comparator__pvfGetIRQSourceHandler(ADC_enMODULE_0,
                                                                           ADC_enSEQ_1,
