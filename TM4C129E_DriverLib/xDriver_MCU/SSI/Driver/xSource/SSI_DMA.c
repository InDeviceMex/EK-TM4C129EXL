...
void SSI__vSetDMARx(SSI_nMODULE enModule, SSI_nDMA enDMAEnableArg)
{
    SSI__vWriteRegister(enModule, SSI_DMACTL_OFFSET,
        (uint32_t) enDMAEnableArg, SSI_DMACTL_RXDMAE_MASK, SSI_DMACTL_R_RXDMAE_BIT);
}

SSI_nDMA SSI__enGetDMARx(SSI_nMODULE enModule)
{
    SSI_nDMA enDMAEnableReg = SSI_enDMA_DIS;
    enDMAEnableReg = (SSI_nDMA) SSI__u32ReadRegister(enModule,
...
{
    SSI__vWriteRegister(enModule, SSI_DMACTL_OFFSET,
        (uint32_t) enDMAEnableArg, SSI_DMACTL_TXDMAE_MASK, SSI_DMACTL_R_TXDMAE_BIT);
}
