                                       ...
                                       uint32_t u32BitFeature)
{
    DMA_CH__vWriteRegister( (uint32_t) DMACH_BASE, (uint32_t) enChannel,
                    DMACH_CHCTL_OFFSET, u32FeatureValue, u32MaskFeature, u32BitFeature);
}
                                ...
                                uint32_t u32BitFeature)
{
    ...
    {
        DMA_CH__vSetPrimaryControlGeneric(enChannel, u32FeatureValue,
                                          u32MaskFeature, u32BitFeature);
    }
    else
    {
        DMA_CH__vSetAlternateControlGeneric(enChannel, u32FeatureValue,
                                            u32MaskFeature, u32BitFeature);
    }
}
...
uint32_t DMA_CH__u32GetPrimaryControlGeneric(DMA_nCH_MODULE enChannel,
                                             ...
                                             uint32_t u32BitFeature)
{
    uint32_t u32CtlReg = 0UL;
    u32CtlReg = DMA_CH__u32ReadRegister( (uint32_t) DMACH_BASE, (uint32_t) enChannel,
                                         DMACH_CHCTL_OFFSET, u32MaskFeature, u32BitFeature);
    return (u32CtlReg);
}
...
{
    uint32_t u32CtlReg = 0UL;
    u32CtlReg = DMA_CH__u32ReadRegister( (uint32_t) DMAALTCH_BASE, (uint32_t) enChannel,
                                     DMACH_CHCTL_OFFSET, u32MaskFeature, u32BitFeature);
    return (u32CtlReg);
}
                                      ...
                                      uint32_t u32BitFeature)
{
    ...
    if(DMA_enCH_CTL_PRIMARY == enChannelStructure)
    {
        u32CtlReg = DMA_CH__u32GetPrimaryControlGeneric(enChannel, u32MaskFeature,
                                                        u32BitFeature);
    }
    else
    {
        u32CtlReg = DMA_CH__u32GetAlternateControlGeneric(enChannel, u32MaskFeature,
                                                          u32BitFeature);
    }
    return (u32CtlReg);
}
