...
void ADC_Comparator__vSetRangeLow(ADC_nMODULE enModule, ADC_nCOMPARATOR enActComp,
                           uint32_t u32CompRangeLow)
{
    ADC_Comparator__vSetGenericRange((uint32_t) enModule, (uint32_t) enActComp,
                              u32CompRangeLow, ADC_DC_CMP_COMP0_MASK, ADC_DC_CMP_R_COMP0_BIT);
}

uint32_t ADC_Comparator__u32GetRangeLow(ADC_nMODULE enModule, ADC_nCOMPARATOR enActComp)
{
    uint32_t u32RangeReg = 0UL;
    u32RangeReg = ADC_Comparator__u32GetGenericRange((uint32_t) enModule, (uint32_t) enActComp,
                                         ADC_DC_CMP_COMP0_MASK, ADC_DC_CMP_R_COMP0_BIT);
    return (u32RangeReg);
}
