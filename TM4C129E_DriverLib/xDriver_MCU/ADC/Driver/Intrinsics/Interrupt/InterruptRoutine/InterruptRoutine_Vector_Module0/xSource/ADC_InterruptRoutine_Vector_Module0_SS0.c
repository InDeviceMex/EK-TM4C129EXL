...
#define ADC_DMA_COMPARATIVE ((uint32_t) ((uint32_t) ADC_enSEQ_SOURCE_DMA << (uint32_t) ADC_enSEQ_0))
#define ADC_Sample_COMPARATIVE ((uint32_t) ((uint32_t) ADC_enSEQ_SOURCE_SAMPLE << (uint32_t) ADC_enSEQ_0))
#define ADC_Comp_COMPARATIVE ((uint32_t) ((uint32_t) ADC_enSEQ_SOURCE_COMP << (uint32_t) ADC_enSEQ_0))
...
void ADC0_SS0__vIRQVectorHandler(void)
{
    ...
    u32RegCompSelect = ADC0_SSOP0_R;
    if(u32Reg & ADC_DMA_COMPARATIVE)
    {
        ADC0_ISC_R = ADC_DMA_COMPARATIVE;
        pfvCallback = ADC_Sample__pvfGetIRQSourceHandler(ADC_enMODULE_0, ADC_enSEQ_0,ADC_enINT_SOURCE_DMA);
        pfvCallback();
    }
    if(u32Reg & ADC_Sample_COMPARATIVE)
    {
        ADC0_ISC_R = ADC_Sample_COMPARATIVE;
        pfvCallback = ADC_Sample__pvfGetIRQSourceHandler(ADC_enMODULE_0, ADC_enSEQ_0,ADC_enINT_SOURCE_SAMPLE);
        pfvCallback();
    }
    if(u32Reg & ADC_Comp_COMPARATIVE)
    {
        ...
        for(u32Pos = (uint32_t) ADC_enMUX_0; u32Pos < (uint32_t) ADC_enMUX_MAX; u32Pos++)
        {
            ...
            {
                u32RegCompMux = ADC0_SSDC0_R >> u32Offset;
                u32RegCompMux &= 0xFUL;
                ...
                if(u32RegCompInterrupt & u32RegCompMuxBit)
                {
                    ...
                    pfvCallback = ADC_Comp__pvfGetIRQSourceHandler(ADC_enMODULE_0, ADC_enSEQ_0,(ADC_nCOMPARATOR) u32RegCompMux);
                    pfvCallback();
                }
            }
            u32Offset += 0x4UL;
