...
void ADC1_SS2__vIRQVectorHandler(void)
{
    ...
    u32RegCompSelect = ADC1_SSOP2_R;
    if(u32Reg & ADC_DMA_COMPARATIVE)
    {
        ADC1_ISC_R = ADC_DMA_COMPARATIVE;
        pfvCallback = ADC_Sample__pvfGetIRQSourceHandler(ADC_enMODULE_1, ADC_enSEQ_2,ADC_enINT_SOURCE_DMA);
        pfvCallback();
    }
    if(u32Reg & ADC_Sample_COMPARATIVE)
    {
        ADC1_ISC_R = ADC_Sample_COMPARATIVE;
        pfvCallback = ADC_Sample__pvfGetIRQSourceHandler(ADC_enMODULE_1, ADC_enSEQ_2,ADC_enINT_SOURCE_SAMPLE);
        pfvCallback();
    }
    if(u32Reg & ADC_Comp_COMPARATIVE)
    {
        ...
        for(u32Pos = (uint32_t) ADC_enMUX_0; u32Pos < (uint32_t) ADC_enMUX_MAX; u32Pos++)
        {
            ...
            {
                u32RegCompMux = ADC1_SSDC2_R >> u32Offset;
                u32RegCompMux &= 0xFUL;
                ...
                if(u32RegCompInterrupt & u32RegCompMuxBit)
                {
                    ...
                    pfvCallback = ADC_Comp__pvfGetIRQSourceHandler(ADC_enMODULE_1, ADC_enSEQ_2,(ADC_nCOMPARATOR) u32RegCompMux);
                    pfvCallback();
                }
            }
            u32Offset += 0x4UL;
