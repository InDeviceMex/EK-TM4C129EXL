...
void UART2__vIRQVectorHandler(void)
{
    ...
    if((uint32_t) UART_enINT_SOURCE_CLEAR_TO_SEND & u32Reg)
    {
        ...
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_CLEAR_TO_SEND);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_RECEIVE & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_RECEIVE;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_RECEIVE);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_TRANSMIT & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_TRANSMIT;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_TRANSMIT);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_RECEIVE_TIMEOUT & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_RECEIVE_TIMEOUT;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_RECEIVE_TIMEOUT);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_FRAME_ERROR & u32Reg)
    {
        ...
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_FRAME_ERROR);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_PARITY_ERROR & u32Reg)
    {
        ...
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_PARITY_ERROR);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_BREAK_ERROR & u32Reg)
    {
        ...
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_BREAK_ERROR);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_OVERRUN_ERROR & u32Reg)
    {
        ...
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_OVERRUN_ERROR);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_END_OF_TRANSMISSION & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_END_OF_TRANSMISSION;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_END_OF_TRANSMISSION);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_BIT9_MODE & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_BIT9_MODE;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_BIT9_MODE);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_DMA_RECEIVE & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_DMA_RECEIVE;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_DMA_RECEIVE);
        pfvCallback();
    }
    if((uint32_t) UART_enINT_SOURCE_DMA_TRANSMIT & u32Reg)
    {
        UART2_ICR_R = (uint32_t) UART_enINT_SOURCE_DMA_TRANSMIT;
        pfvCallback = UART__pvfGetIRQSourceHandler(UART_enMODULE_2, UART_enINTERRUPT_DMA_TRANSMIT);
        pfvCallback();
    }
