                                      ...
                                      GPIO_nFUNCTION enFunctionArg)
{
    ...
    {
        UBase_t uxValueReg = (GPIO_enFUNCTION_GPIO == enFunctionArg) ? 0UL : (UBase_t) enPinMaskArg;
...
{
    GPIO_nERROR enErrorReg;
    enErrorReg = (0UL == (uintptr_t) penFunctionArg) ? GPIO_enERROR_POINTER : GPIO_enERROR_OK;
    if(GPIO_enERROR_OK == enErrorReg)
    {
        enErrorReg = (GPIO_nERROR) MCU__enCheckParams((UBase_t) enPinArg, (UBase_t) GPIO_enPIN_MAX);
    }
