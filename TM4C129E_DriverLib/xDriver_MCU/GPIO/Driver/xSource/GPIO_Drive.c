...
void GPIO__vSetDrive(GPIO_nPORT enPort, GPIO_nPIN enPin, GPIO_nDRIVE enDriveParam)
{
    ...
    if(GPIO_enEXTENDED_DRIVE_EXTENDED == enExtendedDrive)
    {
        ...
        while(0UL != u32PinBit)
        {
            if(u32PinBit & 0x1UL)
            {
                GPIO__vWriteRegister(enPort, GPIO_PC_OFFSET, u32EnExtendedDriveMode, GPIO_PC_EDM0_MASK, u32PinBitPos);
            }
            u32PinBit >>= 1UL;
            u32PinBitPos++;
        }
...
GPIO_nDRIVE GPIO__enGetDrive(GPIO_nPORT enPort, GPIO_nPIN enPin)
{
    ...
    if(GPIO_enEXTENDED_DRIVE_EXTENDED == enExtendedDrive)
    {
        ...
        while(0UL != u32PinBit)
        {
            if(u32PinBit & 0x1UL)
            {
                u32EnExtendedDriveModeAux = GPIO__u32ReadRegister(enPort, GPIO_PC_OFFSET, GPIO_PC_EDM0_MASK, u32PinBitPos);
