...
OS_Queue_Handle_TypeDef OS_Queue__pvGenericCreate(const uint8_t u8QueueLength,
                                             ...
                                             const OS_Queue_nType enQueueTypeArg)
{
    ...
    OS_Queue_Handle_TypeDef pvReturn = (OS_Queue_Handle_TypeDef) 0UL;
    ...
    if(0U < u8QueueLength);
    {
        if(0UL != uxItemSizeArg)
        {
            ...
            pstNewQueue = (OS_Queue_TypeDef*) memalign(OS_ADAPT_BYTE_ALIGNMENT_MASK + 1UL,
                                                 (size_t) uxQueueSizeInBytes * sizeof(OS_Queue_TypeDef));
        #elif defined (__GNUC__ )
            pstNewQueue = (OS_Queue_TypeDef*) malloc((size_t) uxQueueSizeInBytes * sizeof(OS_Queue_TypeDef));
        #endif
        ...
        if(0UL !=  (OS_UBase_t) pstNewQueue)
        {
            ...
            {
                pstNewQueue->ps8Head = (int8_t *) pstNewQueue;
                pstNewQueue->ps8Head += sizeof(OS_Queue_TypeDef);
            }
