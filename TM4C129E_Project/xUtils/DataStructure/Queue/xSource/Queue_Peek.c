...
{
    return SLinkedList__pvGetDataHead((const SLinkedList_TypeDef*)pstQueue);
}

uint32_t Queue__u32GetNMember(const Queue_TypeDef* pstQueue, void** pvData, uint32_t u32Members, uint32_t u32MaxSize)
{
    uint32_t u32SizeReg = 0UL;

    if(((uint32_t) 0UL != (uint32_t) pstQueue) && ((uint32_t) 0UL != (uint32_t) pvData ) && (0UL != u32Members ) && (0UL != u32MaxSize ))
    {
        u32SizeReg = SLinkedList__u32GetNItem((const SLinkedList_TypeDef*)pstQueue, pvData, u32Members, u32MaxSize);
    }
    return u32SizeReg;
}

uint32_t Queue__u32GetAllMember(const Queue_TypeDef* pstQueue, void** pvData, uint32_t u32MaxSize)
{
    uint32_t u32SizeReg = 0UL;

    if(((uint32_t) 0UL != (uint32_t) pstQueue) && ((uint32_t) 0UL != (uint32_t) pvData ) && ((uint32_t) 0UL != u32MaxSize ))
    {
        u32SizeReg = SLinkedList__u32GetAllItem((const SLinkedList_TypeDef*)pstQueue, pvData, u32MaxSize);
    }
    return u32SizeReg;
}
